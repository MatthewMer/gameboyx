...
control_context* MemorySM83::GetControlContext() {
    return &control_ctx;
}
...
u8& MemorySM83::GetIO(const u16& _addr) {
    return IO[_addr - IO_OFFSET];
}

void MemorySM83::SetIO(const u16& _addr, const u8& _data) {
    switch (_addr) {
    case IF_ADDR:
        graphics_ctx.vblank_if_write = true;
        break;
...
/* ***********************************************************************************************************
    CONTROLLER INPUT
*********************************************************************************************************** */
void MemorySM83::SetControlValues(const u8& _data) {
...
void MemorySM83::SetColorPaletteValues(const u8& _data, u32* _color_palette) {
    u8 colors = _data;
    for (int i = 0; i < 4; i++) {
        switch (colors & 0x03) {
        case 0x00:
