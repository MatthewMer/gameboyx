...
GameboyCartridge* GameboyCartridge::instance = nullptr;

GameboyCartridge* GameboyCartridge::getInstance(const game_info& _game_ctx) {
	resetInstance();

	instance = new GameboyCartridge(_game_ctx);
	return instance;
}

GameboyCartridge* GameboyCartridge::getInstance() {
	return instance;
}

void GameboyCartridge::resetInstance() {
	if (instance != nullptr) {
		delete instance;
		instance = nullptr;
	}
...
bool GameboyCartridge::read_basic_header_info(game_info& _game_ctx, vector<u8>& _vec_rom) {
	// read header info -----
...
bool GameboyCartridge::read_rom_to_buffer(const game_info& _game_ctx, std::vector<u8>& _vec_rom) {
...
bool GameboyCartridge::copy_rom_to_rom_folder(game_info& game_ctx, std::vector<u8>& _vec_rom, const string& _new_file_path) {
...
bool GameboyCartridge::read_new_game(game_info& _game_ctx, const string& _path_to_rom) {
	...
	if (!GameboyCartridge::read_rom_to_buffer(_game_ctx, vec_rom)) {
		LOG_ERROR("[emu] Error while reading rom");
		return false;
	}

	if (!GameboyCartridge::copy_rom_to_rom_folder(_game_ctx, vec_rom, s_path_rom_folder)) {
		LOG_WARN("[emu] Fallback to given path");
	}

	if (!GameboyCartridge::read_basic_header_info(_game_ctx, vec_rom)) {
...
void GameboyCartridge::check_and_create_rom_folder() {
