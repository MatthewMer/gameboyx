...
const vector<u8> tex2dVertShader = {
		...
		0x18, 0x00, 0x00, 0x00, 0x03, 0x00, 0x03, 0x00, 0x02, 0x00, 0x00, 0x00, 0xc2, 0x01, 0x00, 0x00,
		...
		0x76, 0x65, 0x72, 0x74, 0x65, 0x78, 0x5f, 0x63, 0x6f, 0x6c, 0x6f, 0x72, 0x00, 0x00, 0x00, 0x00,
		0x05, 0x00, 0x05, 0x00, 0x0b, 0x00, 0x00, 0x00, 0x69, 0x6e, 0x5f, 0x63, 0x6f, 0x6c, 0x6f, 0x72,
		0x00, 0x00, 0x00, 0x00, 0x05, 0x00, 0x06, 0x00, 0x11, 0x00, 0x00, 0x00, 0x67, 0x6c, 0x5f, 0x50,
		...
		0x00, 0x00, 0x00, 0x00, 0x05, 0x00, 0x05, 0x00, 0x18, 0x00, 0x00, 0x00, 0x69, 0x6e, 0x5f, 0x70,
		...
		0x1e, 0x00, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00, 0x48, 0x00, 0x05, 0x00, 0x11, 0x00, 0x00, 0x00,
		...
		0x02, 0x00, 0x00, 0x00, 0x47, 0x00, 0x04, 0x00, 0x18, 0x00, 0x00, 0x00, 0x1e, 0x00, 0x00, 0x00,
		...
		0x16, 0x00, 0x00, 0x00, 0x06, 0x00, 0x00, 0x00, 0x02, 0x00, 0x00, 0x00, 0x20, 0x00, 0x04, 0x00,
		0x17, 0x00, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00, 0x16, 0x00, 0x00, 0x00, 0x3b, 0x00, 0x04, 0x00,
		...
		0x1f, 0x00, 0x00, 0x00, 0x03, 0x00, 0x00, 0x00, 0x0d, 0x00, 0x00, 0x00, 0x36, 0x00, 0x05, 0x00,
		...
		0x0c, 0x00, 0x00, 0x00, 0x3d, 0x00, 0x04, 0x00, 0x16, 0x00, 0x00, 0x00, 0x19, 0x00, 0x00, 0x00,
		0x18, 0x00, 0x00, 0x00, 0x51, 0x00, 0x05, 0x00, 0x06, 0x00, 0x00, 0x00, 0x1c, 0x00, 0x00, 0x00,
		0x19, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x51, 0x00, 0x05, 0x00, 0x06, 0x00, 0x00, 0x00,
		0x1d, 0x00, 0x00, 0x00, 0x19, 0x00, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00, 0x50, 0x00, 0x07, 0x00,
		0x0d, 0x00, 0x00, 0x00, 0x1e, 0x00, 0x00, 0x00, 0x1c, 0x00, 0x00, 0x00, 0x1d, 0x00, 0x00, 0x00,
		0x1a, 0x00, 0x00, 0x00, 0x1b, 0x00, 0x00, 0x00, 0x41, 0x00, 0x05, 0x00, 0x1f, 0x00, 0x00, 0x00,
		0x20, 0x00, 0x00, 0x00, 0x13, 0x00, 0x00, 0x00, 0x15, 0x00, 0x00, 0x00, 0x3e, 0x00, 0x03, 0x00,
...
void VulkanMgr::RenderFrame() {
	...
	commandbuffer_info.flags = VK_COMMAND_BUFFER_USAGE_ONE_TIME_SUBMIT_BIT;
	{
		...
		begin_info.renderArea = { {0, 0}, {graphicsInfo.width, graphicsInfo.height} };
...
void VulkanMgr::BindPipelinesTex2d(VkCommandBuffer& _command_buffer) {
...
void VulkanMgr::UpdateTex2d() {
	...
	region.imageExtent = { graphicsInfo.width, graphicsInfo.height, 1 };
	vkCmdCopyBufferToImage(tex2dCommandBuffer, tex2dStagingBuffer.buffer, tex2dImage.image, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 1, &region);
...
bool VulkanMgr::InitSwapchain(VkImageUsageFlags _flags) {
	...
	graphicsInfo.width = surface_capabilites.currentExtent.width;
	graphicsInfo.height = surface_capabilites.currentExtent.height;
	viewport = { .0f, .0f, (float)graphicsInfo.width, (float)graphicsInfo.height, .0f, 1.f};
	scissor = { {0, 0}, {graphicsInfo.width, graphicsInfo.height} };
...
bool VulkanMgr::InitFrameBuffers() {
	...
	for (u32 i = 0; i < images.size(); i++) {
		...
		framebuffer_info.width = graphicsInfo.width;
		framebuffer_info.height = graphicsInfo.height;
...
bool VulkanMgr::InitPipeline(VkShaderModule& _vertex_shader, VkShaderModule& _fragment_shader, VkPipelineLayout& _layout, VkPipeline& _pipeline, VulkanPipelineBufferInfo& _info) {
...
bool VulkanMgr::LoadBuffer(VulkanBuffer& _buffer, void* _data, u64 _size) {
	if (resizableBar) {
		...
		if (vkCreateCommandPool(device, &create_info, 0, &cmd_pool) != VK_SUCCESS) {
...
bool VulkanMgr::InitImage(VulkanImage& _image, u32 _width, u32 _height, VkFormat _format, VkImageUsageFlags _usage) {
	...
	image_info.extent.height = graphicsInfo.height;
...
bool VulkanMgr::InitTex2dRenderTarget() {
	...
	if (vkCreateCommandPool(device, &cmd_pool_info, 0, &tex2dCommandPool) != VK_SUCCESS) {
		LOG_ERROR("[vulkan] create command pool for tex2d update");
	}

	VkCommandBufferAllocateInfo alloc_info = { VK_STRUCTURE_TYPE_COMMAND_BUFFER_ALLOCATE_INFO };
	alloc_info.commandPool = tex2dCommandPool;
	alloc_info.level = VK_COMMAND_BUFFER_LEVEL_PRIMARY;
	alloc_info.commandBufferCount = 1;
	if (vkAllocateCommandBuffers(device, &alloc_info, &tex2dCommandBuffer) != VK_SUCCESS) {
